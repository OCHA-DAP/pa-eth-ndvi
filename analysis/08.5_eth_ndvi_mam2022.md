# NDVI Analysis for Ethiopia Mar-May 2022
...
mam_pentads = list(range(7, 30 + 1))
mam_file = [
    "ea22" + "{:02}".format(pentad) + "pct.zip" for pentad in mam_pentads
]
```
...
].to_csv(output_dir + "eth_ndvi_mean_median_mam2022.csv", index=False)
```
...
g.set_title(
    f"Percentage of pentads Mar-May 2022 NDVI \n was <={threshold}% of mean NDVI aggregated by median per woreda"
)
g.axis("off")
```
