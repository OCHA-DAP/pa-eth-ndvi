# NDVI Analysis for Ethiopia Jun-Sep 2023
...
jjas_file = ["ea23" + str(pentad) + "pct.zip" for pentad in jjas_pentads]
```
...
jjas23_files = [
    os.path.join(input_dir, "ea23" + str(item) + "pct.tif")
...
gdf_stats_mask[fields].to_csv(
    output_dir + "eth_ndvi_woreda_stats_jjas2023.csv", index=False
)
```
...
].to_csv(output_dir + "eth_ndvi_mean_median_jjas2023.csv", index=False)
```
