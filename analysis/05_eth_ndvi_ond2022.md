# NDVI Analysis for Ethiopia OND 2022
...
ond_pentads = list(range(55, 72 + 1))
ond_file = ["ea22" + str(pentad) + "pct.zip" for pentad in ond_pentads]

for file in ond_file:
...
for file in ond_file:
    file_obj = rasterio.open(input_dir + file.replace("zip", "tif"))
...
g = plot_ndvi_aggr(
    ...
    title=f"Percentage of pentads Oct-Nov 2022 NDVI \n was <={threshold}% of mean NDVI aggregated by mean per woreda",
)
g = plot_ndvi_aggr(
    gdf_stats_mask,
    feature_col="perc_binned_median",
...
].to_csv(output_dir + "eth_ndvi_woreda_stats_ond2022.csv", index=False)
```
...
].to_csv(output_dir + "eth_ndvi_mean_median_ond2022.csv", index=False)
```
