# NDVI Analysis for Ethiopia Oct-Dec 2023
...
ond_pentads = list(range(55, 69 + 1))
ond_file = ["ea23" + str(pentad) + "pct.zip" for pentad in ond_pentads]
```
...
```python
"ea23" + str(ond_pentads) + "*.tif"
```

```python
# list files that start with any value in ma_file
# ond23_files = [f for f in glob.glob(os.path.join(input_dir, "ea23*.tif"))]
ond23_files = [
    # os.path.join(input_dir, "ea23" + str(item) + ".tif")
    glob.glob(os.path.join(input_dir, "ea23" + str(item) + "*.tif"))
    for item in ond_pentads
]
```
...
for file in ond23_files:
    file_obj = rasterio.open(file[0])
    ...
    woreda_stats_mean["".join([n for n in file[0] if n.isdigit()])] = stats_df[
        "mean"
    ]
    woreda_stats_median[
        "".join([n for n in file[0] if n.isdigit()])
    ] = stats_df["median"]
```
...
```python
fields = [
    "number of pentads below 80% using mean",
    "percent of pentads below 80%_mean",
    "perc_binned_mean",
    "number of pentads below 80% using median",
    "percent of pentads below 80%_median",
    "perc_binned_median",
    "include",
]
gdf_stats_mask.rename(
    columns={
        "mean": fields[0],
        "percent_mean": fields[1],
        "perc_binned_mean": fields[2],
        "median": fields[3],
        "percent_median": fields[4],
        "perc_binned_median": fields[5],
        "include": fields[6],
    },
    inplace=True,
)
fields.insert(0, "ADM3_PCODE")
```


```python
gdf_stats_mask[fields].to_csv(
    output_dir + "eth_ndvi_woreda_stats_ond2023.csv", index=False
)
```
...
].to_csv(output_dir + "eth_ndvi_mean_median_ond2023.csv", index=False)
```
